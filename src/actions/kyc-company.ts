...
export async function verifyGstinAction(
  gstin: string
): Promise<ActionState<{ status: string; legal_name?: string }>> {
  const token = await getAccessToken();
  ...
  try {
    const { data, ok } = await apiClient.post<{
      success: boolean;
      data?: { status: string; legal_name?: string };
      message?: string;
    }>("/kyc/company/gstin", { gstin }, { headers: { Authorization: `Bearer ${token}` } });

    if (!ok) return { success: false, error: "GSTIN verification failed" };
...
export async function verifyCinAction(
  cin: string
): Promise<ActionState<{ status: string; company_name?: string }>> {
  const token = await getAccessToken();
  if (!token) return { success: false, error: "Session expired" };
  ...
  try {
    const { data, ok } = await apiClient.post<{
      success: boolean;
      data?: { status: string; company_name?: string };
      message?: string;
    }>("/kyc/company/cin", { cin }, { headers: { Authorization: `Bearer ${token}` } });

    if (!ok) return { success: false, error: "CIN verification failed" };
    return { success: true, data: data.data };
  } catch {
    return { success: false, error: "Network error" };
  }
